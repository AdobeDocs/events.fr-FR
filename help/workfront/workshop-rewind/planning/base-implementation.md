---
title: Workfront Planning - Smart Start - Création de votre implémentation de base
description: Découvrez les bonnes pratiques relatives à la mise en œuvre de Workfront Planning afin de créer une base solide et de permettre la réussite à long terme de l’entreprise.
feature: Strategic Planning
topic: Performance, Content Supply Chain
role: Admin, Developer, Leader, User
level: Beginner, Intermediate, Experienced
doc-type: Event
duration: 3567
last-substantial-update: 2026-03-26T00:00:00Z
jira: KT-19775
exl-id: fa433105-f33e-4aa6-b241-e9b5a9312537
source-git-commit: 460acb3fd1e9b29075cefa07e8d6947d2a61a314
workflow-type: tm+mt
source-wordcount: '209'
ht-degree: 1%

---

# Workfront Planning Smart Start - Création de votre implémentation de base

Rejoignez notre équipe de gestion de produit Adobe qui partage les bonnes pratiques pour commencer votre implémentation de planification Workfront.  Pour plus d’informations sur les prochaines étapes de Workfront Planning, veillez à ajouter un signet à la page [Activité de publication d’Adobe Workfront Planning](https://experienceleague.adobe.com/fr/docs/workfront/using/product-announcements/product-releases/planning-release-activity/planning-release-activity-article-index) dans Experience League.

>[!VIDEO](https://video.tv.adobe.com/v/3477113/?learn=on&enablevpops)

Si vous avez manqué la séance, pas d&#39;inquiétude ! Examinez le [diaporama](https://workfront-experience.s3.us-west-2.amazonaws.com/Training/Guides/Customer+Success+at+Scale/073125+The+Smart+Start+%E2%80%94+Building+Your+Base+Implementation.pdf) et regardez l’enregistrement à la demande pour le suivre.

Avez-vous des idées à partager ou des questions de suivi sur l&#39;événement ? N’hésitez pas à les laisser dans les commentaires de la publication de la communauté [&#128279;](https://experienceleaguecommunities.adobe.com/t5/workfront-discussions/event-follow-up-workfront-planning-the-smart-start-building-your/td-p/767049?profile.language=fr) !

De nouveaux événements sont ajoutés tous les mois. Veillez donc à consulter la page [Événements &#x200B;](https://experienceleague.adobe.com/fr/events) pour connaître les dernières sessions.
